...
def process_csv_with_openai():
    csv_file_path = "COM214_question4_file.csv"
    ...
    for index, row in df.iterrows():
        ...
        full_prompt = f"""You are analyzing student responses to the following question:

        "Anything that I should know about or do to make you more comfortable and able to succeed in this course? (challenges, ideas for improvement, issues outside of class, etc.)"

        Your task is to summarize the student's response in a concise sentence (5–40 words) **only if** it provides helpful, relevant, and course-related feedback (e.g., challenges, support needs, suggestions, or external issues affecting learning).

        If the response is blank, vague, off-topic, irrelevant, or doesn't offer constructive information (e.g., "not yet", "none", "I'm hungry"), just return: **NA**

        Keep summaries in first-person voice if possible and preserve the original intent.
    ...
    output_csv_file_path = f"{csv_file_path}_openai_processed{ext}"
